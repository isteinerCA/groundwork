...
export function SearchExperience({
  ...
  const update = (patch: Partial<SearchFilters>) => {
    ...
    if (patch.gradesCompleted && patch.gradesCompleted.length > 0) setGradeError(false);
  };
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {GRADE_CHIPS.map((grade) => (
            <Chip
              ...
              onClick={() => update({ gradesCompleted: toggle(filters.gradesCompleted, grade) })}
            />
          ))}
        </div>
        {gradeError && (
      ...
      <div className="mt-6 space-y-4">
        ...
        {filters.gradesCompleted.length > 0 && results.length === 0 && (
          <div className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-white p-8 text-center">
            ...
            <p className="mt-2 text-[var(--color-text-muted)]">
              Try removing a category, broadening your price range, or turning off &ldquo;Hide
              unlisted prices.&rdquo;
            </p>
            <button
              type="button"
              onClick={clearAll}
