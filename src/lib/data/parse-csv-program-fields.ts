...
> {
  ...
  const hasCompletingGrades = /completing grades?\s*\d+/i.test(display);
  const isAgeFloorOnly =
    !hasCompletingGrades &&
    /\bmust be\s+\d+\s+by\s+(?:program\s+)?start\b/i.test(display);

  // Standalone minimum-age rows are a floor, not a band — do not layer CSV grade overrides.
  if (isAgeFloorOnly) {
    return parsed;
  }
